...
function fbm(x, y, seed) {
	...
	let freq = 1;
	for (let i = 0; i < 4; i++) {
		val += amp * smoothNoise(x * freq * .1, y * freq * .1, seed + i * 100);
		amp *= .5;
		freq *= 2;
	}
	return val;
}
const init_world = spacetimedb.reducer((ctx) => {
	if (ctx.db.worldState.id.find(1n)) {
		ctx.db.worldState.id.delete(1n);
		for (const t of ctx.db.tile.iter()) ctx.db.tile.id.delete(t.id);
		for (const e of ctx.db.entity.iter()) ctx.db.entity.id.delete(e.id);
		for (const s of ctx.db.structure.iter()) ctx.db.structure.id.delete(s.id);
		for (const ev of ctx.db.eventLog.iter()) ctx.db.eventLog.id.delete(ev.id);
	}
	...
	for (let y = 0; y < 32; y++) for (let x = 0; x < 32; x++) {
		...
		let terrain;
		if (heightVal < .25) terrain = "water";
		else if (heightVal < .32) terrain = "sand";
		else if (heightVal > .75) terrain = "mountain";
		else if (heightVal > .65) terrain = "rock";
		else if (moistureVal > .6) terrain = "forest";
		else if (moistureVal > .5 && heightVal < .4) terrain = "swamp";
		else terrain = "grass";
		const resNoise = noise2d(x * 3.7, y * 2.3, SEED + 1e3);
		let resource = "none";
		if (resNoise > .85) {
	...
	for (const e of [
		...
		{
			name: "Kael the Builder",
			entityType: "builder",
			x: 16,
			y: 16
		},
		{
			name: "Fern the Gatherer",
			entityType: "gatherer",
			x: 24,
			y: 8
		},
		{
			name: "Thane the Guardian",
			entityType: "guardian",
			x: 16,
			y: 24
		}
...
const advance_tick = spacetimedb.reducer((ctx) => {
	...
	const newTime = [
		...
		"dusk",
		"night"
	][Math.floor(tickNum / 24) % 4];
	const newSeason = [
		"spring",
		"summer",
		"autumn",
		"winter"
	][Math.floor(tickNum / 96) % 4];
	const weatherRng = mulberry32(hashSeed(tickNum, 777, 0));
	const weatherRoll = weatherRng();
	let newWeather = ws.weather;
	if (weatherRoll < .15) newWeather = [
		"clear",
		"rain",
		"fog",
		"storm"
	][Math.floor(weatherRng() * 4)];
	const newEpoch = BigInt(Math.floor(tickNum / 384));
	if (newTime !== ws.timeOfDay) ctx.db.eventLog.insert({
		id: 0n,
		tick: newTick,
		message: `Time changed to ${newTime}`,
		entityId: 0n,
		x: 0,
		y: 0,
		eventType: "weather",
	...
	if (newSeason !== ws.season) ctx.db.eventLog.insert({
		id: 0n,
		tick: newTick,
		message: `Season changed to ${newSeason}`,
		entityId: 0n,
		x: 0,
		y: 0,
		eventType: "weather",
	...
	if (newWeather !== ws.weather) ctx.db.eventLog.insert({
		id: 0n,
		tick: newTick,
	...
	for (const ent of entities) {
		...
		if (newTime === "night" && rng() < .6) {
			...
			let canMove = true;
			for (const t of ctx.db.tile.iter()) if (t.x === newX && t.y === newY) {
				if (t.terrain === "water" && ent.entityType !== "wanderer") canMove = false;
			...
			if (canMove) {
				...
				if (rng() < .1) ctx.db.eventLog.insert({
					id: 0n,
					tick: newTick,
		...
		if (ent.state !== newState || ent.energy !== newEnergy) ctx.db.entity.id.update({
			...ent,
			state: newState,
			energy: newEnergy,
			hp: newHp
	...
	for (const s of structures) {
		...
		} else ctx.db.structure.id.update({
			...s,
			durability: newDur
		});
	}
});
const spawn_entity = spacetimedb.reducer({
	name: t.string(),
	entityType: t.string(),
	x: t.i32(),
	y: t.i32()
...
const build_structure = spacetimedb.reducer({
	x: t.i32(),
	y: t.i32(),
	structureType: t.string(),
	builderId: t.u64()
...
const world_event = spacetimedb.reducer({ eventName: t.string() }, (ctx, { eventName }) => {
	const ws = ctx.db.worldState.id.find(1n);
	if (!ws) return;
	const tick = ws.tick;
	const rng = mulberry32(hashSeed(Number(tick), 9999, eventName.length));
	if (eventName === "meteor") {
		const mx = Math.floor(rng() * 32);
		const my = Math.floor(rng() * 32);
		for (const t of ctx.db.tile.iter()) if (Math.abs(t.x - mx) + Math.abs(t.y - my) <= 2) ctx.db.tile.id.update({
			...t,
			terrain: "rock",
			resource: "stone",
			elevation: 8
		...
		ctx.db.eventLog.insert({
			id: 0n,
			tick,
			message: `A meteor struck near (${mx}, ${my})!`,
			entityId: 0n,
			x: mx,
			y: my,
			eventType: "discovery",
		...
		for (const t of ctx.db.tile.iter()) if (rng() < .15) {
		...
		for (let i = 0; i < 3; i++) {
			...
			ctx.db.eventLog.insert({
				id: 0n,
				tick,
				message: `${ename} arrived during migration at (${ex}, ${ey})`,
				entityId: row.id,
				x: ex,
				y: ey,
				eventType: "spawn",
		...
		const entities = [...ctx.db.entity.iter()];
		for (const ent of entities) ctx.db.entity.id.update({
			...ent,
			hp: 100,
			energy: 100
		});
		for (const t of ctx.db.tile.iter()) if (t.terrain === "grass" || t.terrain === "forest") ctx.db.tile.id.update({
			...t,
			fertility: Math.min(100, t.fertility + 20)
		});
		ctx.db.eventLog.insert({
			id: 0n,
			tick,
			message: "A divine blessing washed over the land!",
			entityId: 0n,
			x: 16,
			y: 16,
			eventType: "discovery",
